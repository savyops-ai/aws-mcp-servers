...
ACTIONS = {
    "get_ecs_troubleshooting_guidance": {
        ...
        "required_params": ["access_key", "secret_access_key", "app_name"],
        "optional_params": ["symptoms_description"],
        "transformer": lambda app_name, access_key, secret_access_key, params: {
            "app_name": app_name,
        ...
        "param_descriptions": {
            "access_key": "AWS access key for authenticating the request",
            "secret_access_key": "AWS secret access key for authenticating the request",
    ...
    "fetch_cloudformation_status": {
        ...
        "required_params": ["access_key", "secret_access_key", "stack_id"],
        "optional_params": [],
        "transformer": lambda stack_id, access_key, secret_access_key, params: {
            "stack_id": stack_id,
            "access_key": access_key,
            "secret_access_key": secret_access_key,
        },
        ...
        "example": (
            ...
            '"access_key": "<your-access-key>", '
            '"secret_access_key": "<your-secret-access-key>", '
            '"stack_id": "my-app-stack"}'
        ),
    },
    "fetch_service_events": {
        "func": fetch_service_events,
        "required_params": ["access_key", "secret_access_key", "app_name", "cluster_name", "service_name"],
        ...
        "example": (
            'action="fetch_service_events", parameters={'
            '"access_key": "<your-access-key>", '
            '"secret_access_key": "<your-secret-access-key>", '
            '"app_name": "my-app", '
    ...
    "fetch_task_failures": {
        ...
        "required_params": ["access_key", "secret_access_key", "app_name", "cluster_name"],
        "optional_params": ["time_window", "start_time", "end_time"],
        "transformer": lambda app_name, access_key, secret_access_key, params: {
            "access_key": access_key,
            "secret_access_key": secret_access_key,
...
def _validate_parameters(access_key: str, secret_access_key: str, action: str, app_name: Optional[str], parameters: Dict[str, Any],
) -> None:
    """Validate required parameters (including AWS creds) for the given action."""
    required = ACTIONS[action]["required_params"]

    # Validate app_name if it’s in the required list
    if "app_name" in required and (not app_name or not app_name.strip()):
        raise ValueError(f"app_name is required for action '{action}'")

    # Validate AWS creds if they’re required
    if "access_key" in required and not access_key:
        raise ValueError(f"access_key is required for action '{action}'")
    if "secret_access_key" in required and not secret_access_key:
        raise ValueError(f"secret_access_key is required for action '{action}'")

    # Validate any other required params
    for param in required:
        if param in ("app_name", "access_key", "secret_access_key"):
            continue
...
async def ecs_troubleshooting_tool(
    ...
    try:
        ...
        # Validate parameters (and AWS creds)
        ...
        func_params = action_config["transformer"](
            app_name,
            access_key,
            secret_access_key,
            parameters
