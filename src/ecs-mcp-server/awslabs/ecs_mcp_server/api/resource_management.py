...
async def ecs_resource_management(
    ...
    if resource_type == "cluster":
        if action == "list":
            return await list_clusters(access_key, secret_key, filters)
        elif action == "describe":
            if not identifier:
                raise ValueError("Identifier is required for describe_cluster")
            return await describe_cluster(access_key, secret_key, identifier, filters)

    elif resource_type == "service":
        if action == "list":
            return await list_services(access_key, secret_key, filters)
        elif action == "describe":
            if not identifier:
            ...
            return await describe_service(access_key, secret_key, identifier, filters)

    elif resource_type == "task":
        if action == "list":
            return await list_tasks(access_key, secret_key, filters)
        elif action == "describe":
            if not identifier:
                raise ValueError("Identifier is required for describe_task")
            if "cluster" not in filters:
                raise ValueError("Cluster filter is required for describe_task")
            return await describe_task(access_key, secret_key, identifier, filters)

    elif resource_type == "task_definition":
        if action == "list":
            return await list_task_definitions(access_key, secret_key, filters)
        elif action == "describe":
            if not identifier:
                raise ValueError("Identifier is required for describe_task_definition")
            return await describe_task_definition(access_key, secret_key, identifier)

    elif resource_type == "container_instance":
        if action == "list":
            return await list_container_instances(access_key, secret_key, filters)
        elif action == "describe":
            if not identifier:
            ...
            return await describe_container_instance(access_key, secret_key, identifier, filters)

    elif resource_type == "capacity_provider":
        if action == "list":
            return await list_capacity_providers(access_key, secret_key, filters)
        elif action == "describe":
            if not identifier:
                raise ValueError("Identifier is required for describe_capacity_provider")
            return await describe_capacity_provider(access_key, secret_key, identifier)
